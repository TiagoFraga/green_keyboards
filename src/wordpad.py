...
def keyboard_test(adbcl,filename, output_name):
    initResultsDir()
    ...
    begin_state = output_dir + "/begin_state" + str(script_index) + ".json"
    ...
    end_state = output_dir + "/end_state" + str(script_index) + ".json"
    keyboard.getDeviceResourcesState(end_state)

    keyboard.closeApp(adbcl,package)
    keyboard.exitProfiler(adbcl)
    keyboard.getDataResult(deviceDir,output_dir,script_index,SED_COMMAND,MV_COMMAND)
